...
void Principal::mousePressEvent(QMouseEvent *event)
{
    ...
    case Forma::CIRCUNFERENCIA:
        if (mNumClic == 0) {
            ...
            mCentroCircunferencia = event->pos();
        }
        mNumClic++;
        break;
    }
...
void Principal::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    else if (mForma == Forma::RECTANGULO && mNumClic == 2)
    {
        ...
        // Calcular el radio usando la distancia entre mCentroCircunferencia y mFinal
        int radio = QLineF(mCentroCircunferencia, mFinal).length();

        QPen pincel;
        pincel.setColor(mColor);
        pincel.setWidth(mAncho);
        mPainter->setPen(pincel);
        mPainter->drawEllipse(mCentroCircunferencia, radio, radio);
        mNumClic = 0;
